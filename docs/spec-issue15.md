...
- [ ] Firebase コンソールでパスワードポリシー(8 文字・英字(大/小)・数字)を設定済み
...
- [ ] **Firebase コンソール → Authentication → Settings → パスワードポリシー**を有効化し、
      「8 文字以上」「英字(大文字/小文字のどちらでも可)を含む」「数字を含む」を必須(強制モード)に設定する。
      クライアント側バリデーション(`/[a-zA-Z]/`)と要件を一致させ、
      コンソール側だけ「英小文字必須」にすると `ABCDEFG1` のようなパスワードが
      クライアントは通過後にコンソールで `auth/password-does-not-meet-requirements` になるため注意する。
