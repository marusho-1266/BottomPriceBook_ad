...
> Status: **実装・デプロイ済み(App Check は監視モードまで。enforce は Phase 4 でフォロー)** / 作成: 2026-07-19
...
2. **フィールド検証の上限値**: 以下の提案値で確定
   ...
   - sortOrder: 0 以上の整数(上限なし。`addCategory` が `Date.now()` を
     ソートキーとして使うため 13 桁の値になりうると実装時に判明。
     数値型フィールドは値の大小によらずストレージコストが変わらないため、
     悪用防止の観点では型・非負であることの検証で十分)
3. **書込レート制限**: Firestore ルールでの簡易制限を採用。ユーザーごとの最終書込時刻ドキュメントを
   使い「直前の書込から一定間隔(1 秒)以上」をルールで強制する。Cloud Functions 経由化は行わない。
   **注意**: `rateLimitOk()` は同一バッチ内の複数 create/update は制限しない
   (`ensureBook` のシード書込のように、1 ユーザー操作が複数ドキュメントの書込になるケースを
   許容するため)。したがって実効上限は「1 ドキュメント/秒」ではなく「1 バッチ/秒」であり、
   1 バッチあたりの上限件数(Firestore の仕様上 500)に近い件数まで一度に書けてしまう。
   通常の UI 操作からは発生しない件数のため悪用抑止として許容するが、厳密な
   1 ドキュメント/秒制限が必要になった場合は Cloud Functions 経由やバッチ内件数上限の
   追加検討が必要(「将来スコープ」参照)
...
|---|---|---|
| categories | name, baseUnit, sortOrder | name: string 1〜100 文字 / baseUnit: `['g','ml','個','枚','組','回分']` のいずれか / sortOrder: int かつ 0 以上(上限なし。`Date.now()` をソートキーに使うため) |
...
| books(追加強化) | name, ownerUid, memberUids, bottomWindowMonths, createdAt, deleting を hasOnly で固定 | name: string 1〜100 文字 / memberUids: 要素数上限(例 20)を追加 |
...
- `rateLimits/{uid}`:
  ...
  - read: 本人のみ / delete: 常に拒否(`allow delete: if false;`)。
    delete→create で 1 秒間隔チェックを回避できるバイパス経路になるため。
    掃除は Admin recursiveDelete(退会・削除時)のみが行う
