...
books/{bookId}/members/{uid}
  ...
  inviteCode: string            // 参加に使用したコード(ルール検証用。オーナー自身の doc では省略可)
  joinedAt: Timestamp
```

- **メンバーシップの真実のソースは従来どおり `books.memberUids`**。members doc は
  表示名の保持とルール検証用マーカーを兼ねる
...
1. `books/{bookId}/members/{uid}` を create(`inviteCode` フィールドにコードを格納)
2. `books/{bookId}` を update(`memberUids` に自分の uid を arrayUnion)

ルール側は book update 時に `getAfter()` で同一バッチ内の members doc から
...
- `books` update に「**自分自身の参加**」分岐を追加:
  ...
  - 他フィールド(name / ownerUid / createdAt / bottomWindowMonths)が不変であること
...
- `members`:
  - create: 「本人 + 有効な招待コード」または「既にメンバーである本人」(オーナー補完用)
  - delete: 本人またはオーナー
  - read: book メンバー
