...
- **価格記録**: 編集(価格・内容量・特売フラグ・日付・店舗)と削除を MVP に含める。
  商品詳細の履歴一覧から操作する(全項目のインライン編集は Issue #42 で実装)
...
2. **底値一覧(ホーム)** — 商品ごとの底値・内容量・店舗・単価をカード表示。検索・カテゴリ絞り込み。
   特売由来の底値には特売バッジを表示(内容量表示: `docs/spec-quantity-on-lists.md`)
...
4. **商品詳細** — その商品の価格記録履歴一覧(グラフは将来)、店舗別の底値、
   ...
   編集対象は価格・内容量・単位・特売・日付・店舗(Issue #42)。
   商品マスタ(名前・カテゴリ・メモ)の編集は設定の商品管理へ誘導する(Issue #40)
5. **カテゴリ内比較** — カテゴリを選ぶと、カテゴリ内の全価格記録(商品へ集約しない)を
   「基準単位あたり単価」の昇順でランキング表示。各行に価格・内容量・店舗名・記録日・特売バッジを表示。
   底値の対象期間(N ヶ月)設定を適用し、表示は上位50件まで(超過分は「他 N 件」注記)。
   単価換算できない記録は単価不明として末尾に表示
   (詳細: `docs/spec-issue1-category-comparison.md` /
   内容量表示: `docs/spec-quantity-on-lists.md`)
6. **設定** — カテゴリ管理・店舗管理(参照中は削除不可)・**商品管理**(登録・編集・削除・メモ。
   Issue #40)・底値の対象期間(N ヶ月)・共有・ログアウト・**アカウント削除(退会)**(Issue #13)
...
- [x] 価格記録をあとから編集・削除できる(価格・内容量・単位・特売・日付・店舗。Issue #42)
